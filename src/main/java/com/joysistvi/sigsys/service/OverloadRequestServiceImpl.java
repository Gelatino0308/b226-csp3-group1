...
public class OverloadRequestServiceImpl implements OverloadRequestService {
    ...
    @Override public List<OverloadRequest> getStudentRequests(int studentId) {
        return studentId > 0 ? requestRepository.getByStudent(studentId) : java.util.Collections.emptyList();
    }
    @Override public Integer getApprovedMaxUnits(int studentId) {
        return studentId > 0 ? requestRepository.getApprovedUnits(studentId) : null;
    }
    @Override public boolean review(int requestId, String status, int adminId) {
        return requestId > 0 && adminId > 0 && status != null
                && status.trim().toUpperCase().matches("APPROVED|DISAPPROVED")
