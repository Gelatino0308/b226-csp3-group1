...
public class CourseSectionServiceImpl implements CourseSectionService {
    ...
    public CourseSection getSectionById(int sectionId) {
        return sectionId > 0 ? sectionRepository.getSectionById(sectionId) : null;
    }

    @Override
    public List<CourseSection> getSectionsByFaculty(int facultyId) {
        return facultyId > 0 ? sectionRepository.getSectionsByFacultyId(facultyId) : java.util.Collections.emptyList();
    }

    @Override
    public List<CourseSection> getSectionsByCourse(int courseId) {
        return courseId > 0 ? sectionRepository.getSectionsByCourseId(courseId) : java.util.Collections.emptyList();
    }

    @Override
    public boolean deleteSection(int sectionId) {
        return sectionId > 0 && sectionRepository.deleteSection(sectionId);
    }
}
